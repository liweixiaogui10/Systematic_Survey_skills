...
**目标：** 以主题而非文献为轴心，整合元数据统计与内容分析，识别国内外差异。
...
> 回答"**有哪些方法？各自优缺点是什么？**"
> 标志：有分类体系（Taxonomy）+ 横向比较表

```
# [研究方向]系统调研：[副标题]

## 摘要（400-600字：背景、检索方法、分类框架、主要发现、对比结论、趋势）
**关键词：**
...
## 3. 分类体系（Taxonomy）
（核心创新：提出本综述的分类框架，用树形图描述，引用高被引综述建立框架）
### 3.1 分类维度一：[按 X 分类]
### 3.2 分类维度二：[按 Y 分类]
...
### 4.2 代表性方法 — 国际研究
### 4.3 本类别横向比较表
  | 方法 | 访问类型 | 训练要求 | 适用场景 | 代表结果 | 局限 |

## 5. [类别B]：...
## 6. [类别C]：...

## 7. 全局横向比较分析
（所有方法的大表，按指标/数据集/成本多维对比）
### 7.1 国内外研究对比矩阵

## 8. 研究趋势与前沿方向
（基于近2年高 fwci 文献，3-5个趋势，需文献数据支撑）
...
> 标志：PRISMA 流程 + 核心争论 + 批判性分析

```
# [研究方向]系统综述：[副标题]

## 摘要（结构化摘要：背景/目的/方法/结果/结论，各50-100字）
**关键词：**
...
### 2.1 检索策略（可复现，含检索日期）
### 2.2 纳入/排除标准（明确表格列出）
### 2.3 PRISMA 筛选流程（四阶段数量表）
### 2.4 质量评估方法（Cochrane / Newcastle-Ottawa / 自定义）
### 2.5 数据提取与合并方法

## 3. 历史发展脉络
（按时间线梳理：奠基性研究 → 关键转折点 → 当前主流范式）
...
## 5. 核心争论点（Key Debates）
（识别领域内存在分歧的核心问题，每条争论：正方证据 vs. 反方证据 → 作者判断）
...
## 1. 引言（为何需要摸清边界）
## 2. 方法（宽松纳入标准 + 检索策略）
## 3. 领域全景图（研究分布：年份/地区/方法类型）
## 4. 研究维度梳理（有哪些子方向/研究角度）
## 5. 跨学科关联（与哪些领域交叉）
## 6. 研究空白地图（哪些维度缺少研究）
## 7. 未来研究机会（可进一步做 Systematic Review 的方向）
## 8. 结论
## 参考文献
```
...
#### 6.1 公共部分：文献检索方法（所有类型通用）

```
## 2. 文献检索方法

### 2.1 检索数据库
Sciverse 学术数据库（含多源聚合，覆盖 arXiv、ACL Anthology、PubMed、IEEE Xplore 等；截至 [检索日期，如 2026-05-20] 收录 516 亿+知识记录）。

### 2.2 检索策略（可复现）

**语义检索（agentic-search）关键词组：**
（逐一列出实际使用的查询串，如）
- EN-1: "hallucination detection large language models survey"
- EN-2: "LLM factual consistency evaluation methods"
- ZH-1: "大语言模型幻觉检测方法综述"

**结构化 meta-search 条件：**
（逐一列出实际使用的过滤条件，如）
- 高引文献：query="LLM hallucination", citation_count≥200, year≥2015
- 国内文献：language IN ["zh","chinese"], year≥2018
- 最新文献：year≥2023, citation_count≥3

**引用链追踪：** 从 Top-[N] 高引文献的 references 字段提取高频 doc_id（出现≥3次）

### 2.3 纳入/排除标准

**纳入：** 发表年 [起始年]-[结束年]；与主题直接相关；发表于顶级期刊/会议或知名预印本；引用量满足年龄调整阈值

**排除：** 主题相关性低（摘要评分 < 阈值）；重复发表；纯技术报告类；无实验验证的纯理论猜测
...
## 3. 定义与分类体系
（综述领域核心概念的定义与分类，引用高被引综述文献建立分类框架）

## 4. 国内研究现状
### 4.1 [主题A]（2-4段，每段2-4篇文献支撑）
### 4.2 [主题B]
### 4.3 国内研究小结（代表机构/团队、主要贡献格局）

## 5. 国际研究进展
### 5.1 [主题A]
### 5.2 [主题B]
### 5.3 国际研究小结（顶级期刊/会议主流方向）

## 6. 国内外比较分析
（含5维度对比矩阵 + 相似点 + 差异点 + 互补机会）

## 7. 研究趋势与前沿方向
（基于近2年高 fwci 文献，提炼3-5个趋势，需有文献数据支撑）
